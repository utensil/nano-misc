...
function document_ready()
{
	...
	function functioncontent_cornerize(o)
	{
		//设置 content 圆角
	...
	//将未分类的function的事件绑定
	jMenuItem.next('.functionmenu').children('.non-category').children('.functionitem')
	.click(functionitem_click)
	.one('click', functioncontent_cornerize);
	
	//将 functionitem 单击事件的绑定推迟至 categoryitem 被单击之后，并且设置 content 圆角。推迟绑定事件是为是优化性能
	jCategoryItem.one('click',function(e){
		//functionitem 单击事件
		$(e.target).next('.category').children('.functionitem')
		.click(functionitem_click)
		.one('click', functioncontent_cornerize);
	});
	...
	$("body > .mainmenu > .functionmenu .content").draggable();




	var itemsToDrag = jMenuItem.add(jCategoryItem).add("body > .mainmenu > .functionmenu .functionitem");
	
	itemsToDrag.draggable({
		 revert: 'invalid',
		 //stack: { group: '.parent .self', min: 1 },
		 zIndex: 1200
	 });
	
	$("body > .mainmenu > .menuitem").droppable({
		tolerance: 'pointer',
		drop: function(ev, ui){
			alert("dropped!");
		}
...
/*
	var itemsToSortString = "body > .mainmenu, body > .mainmenu > .functionmenu, body > .mainmenu > .functionmenu > .category";
	
	$(itemsToSortString).sortable({
		items: '> *:not(.non-category)'
		//connectWith: itemsToSortString.split(',')
	});	
*/

	
	$('.browse_button').toggle(function() {
		
		var iframe = '<iframe src="' + $(this).attr('url') +'"></iframe>';
		$(this).after(iframe);
